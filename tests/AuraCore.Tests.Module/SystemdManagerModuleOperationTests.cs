...
public class SystemdManagerModuleOperationTests
{
    ...
    {
        if (OperatingSystem.IsLinux()) return; // platform-specific assertion only on non-Linux
#pragma warning disable CA1416
        IOperationModule module = new SystemdManagerModule();
...
#pragma warning restore CA1416
    ...
    {
        if (!OperatingSystem.IsLinux()) return; // skip Skipped-path assertion off-Linux
#pragma warning disable CA1416
        IOperationModule module = new SystemdManagerModule();
        var result = await module.RunOperationAsync(
            new OptimizationPlan(module.Id, Array.Empty<string>()),
