...
public sealed class MemoryTrendTracker : IDisposable
{
    ...
    public void Start()
    {
        if (_running) return;
        _running = true;
        _timer = new System.Threading.Timer(Sample, null, TimeSpan.Zero, TimeSpan.FromSeconds(5));
    }
        ...
        try
        {
            ...
            foreach (var proc in System.Diagnostics.Process.GetProcesses())
            {
                try
                {
                    ...
                    // Leak detection: need at least 6 samples
    ...
    private static void AnalyzeTrend(ProcessTrend trend)
    {
        ...
        // Suspected leak: >75% of samples show increase AND total growth > 5MB
        var totalGrowthMb = totalGrowth / (1024.0 * 1024);
        var timeSpanMinutes = intervals * 5.0 / 60.0; // 5 sec per sample
